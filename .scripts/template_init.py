...
3. Replaces all instances of "pythonsteps" with the module name
4. Renames the src/pythonsteps directory to src/{modulename}
...
def main():
    ...
    if module_name == "pythonsteps":
        print("Module name is already 'pythonsteps', no changes needed.")
        return
    ...
    for file_path in files_to_process:
        if find_and_replace_in_file(file_path, "pythonsteps", module_name, args.dry_run):
            files_changed += 1
    ...
    if rename_directory("pythonsteps", module_name, args.dry_run):
