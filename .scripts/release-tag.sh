...
") || exit
  ...
  set -v
  gh release create $p "v${v}" --title "v$(python -c "try: import tomllib; except ImportError: import tomli as tomllib; print(tomllib.load(open('pyproject.toml', 'rb'))['project']['version'])")" --notes "" && exit
fi
